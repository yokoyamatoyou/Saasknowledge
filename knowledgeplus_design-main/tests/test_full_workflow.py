...
from core import faq_utils  # noqa: E402
...
def test_build_and_generate_faq(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        faq_utils, "BASE_KNOWLEDGE_DIR", tmp_path / "knowledge_base", raising=False
    )
    ...
    class MockClient:
        def __init__(self):
            ...
            self.embeddings.create.return_value = MagicMock(
                data=[MagicMock(embedding=[0.1] * 5)]
            )
            self.chat = MagicMock()
            self.chat.completions.create.return_value = MagicMock(
                choices=[
                    MagicMock(
                        message=MagicMock(
                            content='{"image_type": "図面", "main_content": "dummy", "keywords": ["x"], "description_for_search": "d"}'
                        )
                    )
                ]
            )
    ...
    def fake_generate(name, max_tokens=1000, num_pairs=3, client=None, source=None):
        ...
        (tmp_path / "knowledge_base" / name / "embeddings" / "faq1.pkl").write_bytes(
            b"0"
        )
