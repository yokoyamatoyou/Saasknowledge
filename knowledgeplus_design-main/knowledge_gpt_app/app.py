...
try:
    ...
    logger.error(f"自作モジュールのインポートに失敗しました: {e}", exc_info=True)
...
def get_embedding(text, model=None, client=None):
    ...
    try:
        ...
        response = client.embeddings.create(
            model=effective_model, input=text_to_embed, dimensions=EMBEDDING_DIMENSIONS
        )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは文書分類の専門家です。与えられたテキストからドキュメントの種類を判別してください。",
                },
                {
                    "role": "user",
                    "content": f"""
        ...
        return {
            "doc_type": "一般文書",
            "confidence": 0.5,
            "reasoning": "自動判別に失敗したためデフォルト値を使用します。",
        }
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは意味ベースのテキストチャンク分割の専門家です。GPT-4.1-miniでナレッジ検索を行うことを想定したパラメータを推奨してください。",
                },
                {
                    "role": "user",
                    "content": f"""
...
def create_run_script():
    try:
        ...
        with open(
            script_dir / "start_app.sh", "w", encoding="utf-8", newline="\n"
        ) as f:
    ...
    try:
        with tempfile.TemporaryDirectory() as export_dir_temp:
            ...
            zip_file_path = shutil.make_archive(
                str(archive_base_name), "zip", root_dir=RAG_BASE_DIR, base_dir=kb_name
            )
...
def segment_text_by_meaning(
    ...
    try:
        ...
        response_analysis = client.chat.completions.create(
            ...
            response_format={"type": "json_object"},
            messages=[
                {"role": "system", "content": "テキスト分析と構造化の専門家。"},
        ...
        response_segment = client.chat.completions.create(
            ...
            response_format={"type": "json_object"},
            messages=[
                {"role": "system", "content": "テキストを意味のまとまりで適切に分割。"},
        ...
        if (
            ...
            if isinstance(result[first_key], list) and all(
                isinstance(s, str) for s in result[first_key]
            ):
...
def create_overlapping_chunks(
    segments, overlap_ratio_percent=15, max_chunk_tokens=1000
):
    if not segments:
        return []
    chunks_text = []
    ...
    for i, segment_text in enumerate(segments):
        ...
        if not current_chunk_text:
            ...
            if estimate_tokens(potential_chunk_text) > max_chunk_tokens:
                ...
                sentences_in_completed_chunk = re.split(
                    r"(?<=[。！？.!?])\s*", current_chunk_text.strip()
                )
                sentences_in_completed_chunk = [
                    s for s in sentences_in_completed_chunk if s.strip()
                ]
                ...
                if sentences_in_completed_chunk:
                    ...
                    for sent in reversed(sentences_in_completed_chunk):
                        ...
                        if (
                            accumulated_len >= target_overlap_chars
                            and len(temp_overlap) > 0
                        ):
                            break
                    overlap_text_content = " ".join(reversed(temp_overlap)).strip()
                current_chunk_text = (
                    (overlap_text_content + "\n" + segment_text).strip()
                    if overlap_text_content
                    else segment_text
                )
            else:
        ...
        while estimate_tokens(current_chunk_text) > max_chunk_tokens:
            ...
            for idx, sentence_part in enumerate(sentences_to_split):
                ...
                if estimate_tokens(tentative_new_chunk) <= max_chunk_tokens:
                    ...
                    if idx == 0:
                        newly_formed_chunk_part = sentence_part[
                            :max_chunk_chars
                        ].strip()
            ...
            if newly_formed_chunk_part:
                ...
                remaining_sentences_text = "\n".join(
                    sentences_to_split[split_after_sentence_index:]
                ).strip()
                if remaining_sentences_text:
                    sentences_in_new_chunk = re.split(
                        r"(?<=[。！？.!?])\s*", newly_formed_chunk_part.strip()
                    )
                    sentences_in_new_chunk = [
                        s for s in sentences_in_new_chunk if s.strip()
                    ]
                    forced_overlap_content = ""
                    if sentences_in_new_chunk:
                        target_forced_overlap_chars = int(
                            len(newly_formed_chunk_part) * forced_overlap_ratio
                        )
                        temp_forced_overlap = []
                        ...
                        for sent_f in reversed(sentences_in_new_chunk):
                            ...
                            if (
                                accumulated_forced_len >= target_forced_overlap_chars
                                and len(temp_forced_overlap) > 0
                            ):
                                break
                        forced_overlap_content = " ".join(
                            reversed(temp_forced_overlap)
                        ).strip()
                    current_chunk_text = (
                        (
                            forced_overlap_content + "\n" + remaining_sentences_text
                        ).strip()
                        if forced_overlap_content
                        else remaining_sentences_text
                    )
                else:
                    current_chunk_text = ""
            else:
    ...
    try:
        metadata_kb = {
            ...
            "original_filename": st.session_state.get(
                "_last_uploaded_filename_for_doc_type", "N/A"
            ),
        }
...
def generate_folder_structure(text_sample, document_type, client=None):
    if client is None:
        ...
        if client is None:
            return {
                "folder_name": f"doc_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                "base_filename": "chunk",
                "description": "クライアント無",
            }
    if not text_sample or not text_sample.strip():
        return {
            "folder_name": f"doc_empty_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "base_filename": "empty_chunk",
            "description": "テキストサンプル空",
        }
    try:
        prompt = f"""
        ...
        result["folder_name"] = re.sub(
            r"\s+",
            "_",
            result.get("folder_name", f"folder_{datetime.now().strftime('%Y%m%d')}"),
        )
        result["folder_name"] = re.sub(safe_chars_pattern, "_", result["folder_name"])
        result["base_filename"] = re.sub(
            r"\s+", "_", result.get("base_filename", "chunk")
        )
        result["base_filename"] = re.sub(
            safe_chars_pattern, "_", result["base_filename"]
        )
        return result
    except Exception as e:
        ...
        return {
            "folder_name": f"doc_err_{timestamp_str}",
            "base_filename": f"chunk_err_{timestamp_str}",
            "description": f"エラーのためデフォルト: {e}",
        }
...
def expand_search_query(query, client=None):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=GPT4_MINI_MODEL,
            messages=[
                {"role": "system", "content": "検索クエリ最適化専門家。"},
                {"role": "user", "content": prompt},
            ],
        )
...
def search_multiple_knowledge_bases(
    ...
    if client is None:
        ...
        if client is None:
            ...
            logger.error(
                "OpenAI client failed to initialize in search_multiple_knowledge_bases."
            )
            return [], True
    if not kb_names:
    ...
    try:
        ...
        with st.status("フォルダ構造を生成中...", expanded=False) as status_fs:
            folder_structure = generate_folder_structure(
                text[: min(len(text), 5000)], document_type, client
            )
        ...
        with st.status("テキストを意味単位で分割中...", expanded=False) as status_seg:
            segments = segment_text_by_meaning(
        ...
        embedding_model_for_kb = st.session_state.get(
            "embedding_model", EMBEDDING_MODEL
        )
        ...
        for i, chunk_content in enumerate(chunks):
            ...
            with st.status(
                f"チャンク {chunk_id_str}/{total_chunks} を処理中...", expanded=False
            ) as status_item:
                ...
                mini_context = optimize_chunk_for_mini(
                    chunk_content, document_type, metadata, client
                )
                ...
                if embedding is None:
                    st.warning(
                        f"チャンク {chunk_id_str} の埋め込みベクトル生成に失敗しました。このチャンクのベクトル情報はスキップされます。"
                    )
...
if app_mode == "ナレッジ構築":
    ...
    with tab1:
        ...
        kb_name_input = st.text_input(
            ...
            value=st.session_state.get(
                "knowledge_base_name", f"kb_{datetime.now().strftime('%Y%m%d')}"
            ),
        )
        ...
        embedding_model_idx = (
            embedding_model_options.index(current_embedding_model)
            if current_embedding_model in embedding_model_options
            else 0
        )
        selected_embedding_model = st.selectbox(
            "埋め込みモデル*", embedding_model_options, index=embedding_model_idx
        )
        st.session_state["embedding_model"] = selected_embedding_model
        uploaded_file = st.file_uploader(
            "対象ファイルを選択 (PDF, DOCX, XLSX, TXT)*",
                        ...
                        if client_for_detection:
                            detection_result = detect_document_type(
                                file_content_for_detection[
                                    : min(len(file_content_for_detection), 10000)
                                ],
                            ...
                            st.success(
                                f"文書タイプ自動判別: {detection_result.get('doc_type')} (確信度: {detection_result.get('confidence', 0):.2f})"
                            )
        ...
        if (
            document_type_selection_from_state
            and document_type_selection_from_state in document_type_options
        ):
            doc_type_final_idx = document_type_options.index(
                document_type_selection_from_state
            )
        final_document_type_selected = st.selectbox(
            "文書タイプ*",
            document_type_options,
            index=doc_type_final_idx,
            help="文書の種類を選択または自動判別結果を修正してください。",
        )
        st.sidebar.header("チャンク分けパラメータ")
        overlap_ratio_ui_val = st.sidebar.slider(
        ...
        current_sudachi_display_ui = next(
            (
                k
                for k, v in sudachi_options_map_ui.items()
                if v == current_sudachi_mode_val_ui
            ),
            sudachi_display_options_ui[2],
        )
        sudachi_mode_display_selected_ui = st.sidebar.selectbox(
            "形態素解析の粒度 (日本語)",
            sudachi_display_options_ui,
            index=sudachi_display_options_ui.index(current_sudachi_display_ui),
        )
        st.session_state["sudachi_mode"] = sudachi_options_map_ui[
            sudachi_mode_display_selected_ui
        ]
        st.sidebar.subheader("詳細設定")
        max_chunk_size_ui_val = st.sidebar.slider(
            "最大チャンクサイズ (トークン)",
            200,
            8000,
            st.session_state.get("max_chunk_size", 1000),
            100,
            help="1チャンクあたりの最大トークン数。モデルのコンテキスト長も考慮してください。",
        )
        st.session_state["max_chunk_size"] = max_chunk_size_ui_val
        forced_overlap_ratio_ui_val = st.sidebar.slider(
            "強制分割時のオーバーラップ率 (%)",
            0,
            50,
            st.session_state.get("forced_overlap_ratio", 25),
            5,
            help="チャンクが最大サイズを超えた場合に強制分割する際のオーバーラップ。",
        )
            ...
            with st.sidebar.expander("GPTによる推奨設定案を見る", expanded=False):
                ...
                if st.button("この推奨設定を適用", key="apply_rec_params_sidebar"):
                    st.session_state["overlap_ratio"] = rec_p.get(
                        "overlap", st.session_state["overlap_ratio"]
                    )
                    st.session_state["sudachi_mode"] = rec_p.get(
                        "sudachi_mode", st.session_state["sudachi_mode"]
                    )
                    st.rerun()
        st.markdown("---")
        if uploaded_file is not None and kb_name_input:
            if st.button(
                "◎ ナレッジベース構築実行",
                key="execute_chunking_button_main",
                type="primary",
                help="上記の設定でナレッジベースの構築を開始します。",
            ):
                ...
                if main_content_for_chunking:
                    ...
                    if not client_for_pipeline:
                        ...
                        if auto_detect_doc_type and not st.session_state.get(
                            "recommended_params"
                        ):
                            if (
                                ...
                                with st.spinner("最適パラメータを分析中..."):
                                    rec_params_val = get_recommended_parameters(
                                        main_content_for_chunking[
                                            : min(len(main_content_for_chunking), 10000)
                                        ],
                                    ...
                                    st.info(
                                        f"推奨オーバーラップ率: {rec_params_val.get('overlap')}% / 推奨Sudachiモード: {rec_params_val.get('sudachi_mode')}"
                                    )
        ...
        if st.session_state.get("processed_chunks"):
            ...
            st.success(
                f"ナレッジベース「{st.session_state['knowledge_base_name']}」に {len(chunks_to_display)}個のチャンクを保存しました。"
            )
            st.subheader("作成されたチャンクのプレビュー")
            for i, chunk_item_disp in enumerate(chunks_to_display[:5]):
                with st.expander(
                    f"チャンク {chunk_item_disp.get('id', i+1)}: {chunk_item_disp.get('filename', '')} (トークン: {chunk_item_disp.get('token_count')})"
                ):
                    st.markdown(
                        f"**要約**: {chunk_item_disp.get('meta_info', {}).get('summary', 'なし')}"
                    )
                    tab_c_cont, tab_c_meta = st.tabs(["内容プレビュー", "メタ情報"])
                    with tab_c_cont:
                        st.text_area(
                            "内容 (先頭500文字)",
                            chunk_item_disp.get("content", "")[:500] + "...",
                            height=150,
                            key=f"chunk_disp_content_{chunk_item_disp.get('id')}",
                        )
                    with tab_c_meta:
    ...
    with tab2:
        ...
        if kb_list_for_mng:
            ...
            if not df_kb.empty:
                ...
                df_kb_display.rename(
                    columns={
                        "name": "KB名",
                        "num_chunks": "チャンク数",
                        "doc_type": "文書タイプ",
                        "embedding_model": "埋込モデル",
                        "created_at": "作成日時",
                        "updated_at": "最終更新日時",
                    },
                    inplace=True,
            ...
            st.markdown("---")
            selected_kb_to_manage = st.selectbox(
                "管理するナレッジベースを選択",
                [kb["name"] for kb in kb_list_for_mng],
                index=None,
            ...
            if selected_kb_to_manage:
                ...
                with col_mng1:
                    if st.button(
                        f"↓ 「{selected_kb_to_manage}」をエクスポート",
                        ...
                        with st.spinner(
                            ...
                            if export_zip_data:
                                st.download_button(
                                    label=f"◎ {selected_kb_to_manage}_export.zip をダウンロード",
                                    data=export_zip_data,
                                    file_name=f"{selected_kb_to_manage}_export.zip",
                ...
                with col_mng2:
                    if st.button(
                        f"× 「{selected_kb_to_manage}」を削除",
                        type="secondary",
                        key=f"delete_kb_btn_{selected_kb_to_manage}",
                        use_container_width=True,
                        help="この操作は元に戻せません！",
                    ):
                        confirm_delete = st.checkbox(
                            f"本当に「{selected_kb_to_manage}」を削除しますか？",
                            key=f"confirm_del_{selected_kb_to_manage}",
                        )
                        if confirm_delete:
                            if selected_kb_to_manage in st.session_state.get(
                                "search_engines", {}
                            ):
                                del st.session_state.search_engines[
                                    selected_kb_to_manage
                                ]
                                logger.info(
                                    f"検索エンジンキャッシュから {selected_kb_to_manage} を削除しました。"
                                )
                            try:
                                shutil.rmtree(RAG_BASE_DIR / selected_kb_to_manage)
                                st.success(
                                    f"ナレッジベース '{selected_kb_to_manage}' を完全に削除しました。"
                                )
                                if (
                                    st.session_state.get("selected_kb_option")
                                    == selected_kb_to_manage
                                ):
                                    st.session_state.selected_kb_option = None
                                if selected_kb_to_manage in st.session_state.get(
                                    ...
                                    st.session_state.selected_kbs.remove(
                                        selected_kb_to_manage
                                    )
                                st.rerun()
                            except Exception as e:
                                st.error(
                                    f"ナレッジベース「{selected_kb_to_manage}」の削除中にエラーが発生しました: {e}"
                                )
                                logger.error(
                                    f"KB削除エラー ({selected_kb_to_manage}): {e}",
                                    exc_info=True,
                                )
        else:
            st.info(
    ...
    with st.expander("⚡ ワンクリック起動スクリプトを作成"):
        if st.button(
            ...
            st.info(
                "生成されたスクリプト（start_app.bat / start_app.sh）をダブルクリック（または実行）するだけで、次回から簡単にアプリを起動できます。初回起動時にAPIキーの入力を求められることがあります（.envファイルに保存されます）。"
            )
    ...
    if kb_names_for_search_ui:
        ...
        current_selected_kb_option_val = st.session_state.get(
            "selected_kb_option", all_kb_option_display_ui
        )
        if current_selected_kb_option_val not in options_for_select_ui:
            logger.warning(
                f"セッションのKBオプション '{current_selected_kb_option_val}' が見つかりません。デフォルトを使用します。"
            )
        ...
        selected_kb_option_idx = options_for_select_ui.index(
            current_selected_kb_option_val
        )
        selected_kb_display_option_ui = st.sidebar.selectbox(
            "検索対象ナレッジベース",
            options_for_select_ui,
        ...
        loaded_engine_names = [
            name
            for name, engine in st.session_state.get("search_engines", {}).items()
            if engine is not None
        ]
    ...
    if generate_gpt_answer_flag_ui:
        ...
        current_persona_name_display_kb_gpt = next(
            (
                p["name"]
        ...
        st.session_state["persona"] = persona_map_kb_gpt[
            selected_persona_name_kb_gpt_ui
        ]
        temp_kb_gpt_ui = st.sidebar.slider(
            "温度",
            0.0,
            1.0,
        ...
        resp_len_idx_kb_gpt = (
            resp_len_options_kb_gpt.index(current_resp_len_kb_gpt)
            if current_resp_len_kb_gpt in resp_len_options_kb_gpt
            else 1
        )
        resp_len_kb_gpt_ui = st.sidebar.radio(
            "応答の長さ",
            resp_len_options_kb_gpt,
            index=resp_len_idx_kb_gpt,
            key="resp_len_kb_gpt",
            horizontal=True,
        )
        st.session_state["response_length"] = resp_len_kb_gpt_ui
    search_query_input_val = st.text_input(
        "検索クエリを入力してください", placeholder="例: 最新の休暇制度について教えて"
    )
    if st.button(
        "◎ 検索実行",
        type="primary",
        key="search_execute_main_button",
        use_container_width=True,
    ):
        if not search_query_input_val.strip():
            ...
            for kb_name_check_search in st.session_state["selected_kbs"]:
                if get_search_engine(kb_name_check_search) is not None:
                    ...
                    st.warning(
                        f"ナレッジベース '{kb_name_check_search}' の検索エンジン準備に失敗。このKBは検索対象外となります。"
                    )
            if not ready_to_search_kbs_list:
                st.error("検索可能なナレッジベースがありません。")
            else:
                with st.spinner(
                    f"「{search_query_input_val}」で検索中 (対象KB: {', '.join(ready_to_search_kbs_list)})..."
                ):
                    ...
                    if generate_gpt_answer_flag_ui and search_results_data:
                        with st.spinner("検索結果に基づいてGPTが回答を生成中..."):
                            ...
                            for idx_res, res_item_gpt in enumerate(search_results_data):
                                meta_gpt = res_item_gpt.get("metadata", {}).get(
                                    "meta_info", {}
                                )
                                ...
                                search_context_str_gpt += (
                                    f"- 要約: {meta_gpt.get('summary', 'なし')}\n"
                                )
...
"""
                            gpt_answer_text = (
                                st.session_state.chat_controller.generate_gpt_response(
                    ...
                    for i_disp, res_detail_disp in enumerate(search_results_data):
                        ...
                        file_name_disp = res_detail_disp.get("metadata", {}).get(
                            "filename", "N/A"
                        )
                        chunk_id_disp = res_detail_disp.get("metadata", {}).get(
                            "id", "N/A"
                        )
                        expander_title = f"結果 {i_disp+1}: {file_name_disp} (ID: {chunk_id_disp}) / KB: {kb_name_disp} / 類似度: {similarity_disp:.3f}"
                        with st.expander(expander_title):
                            meta_info_disp = res_detail_disp.get("metadata", {}).get(
                                "meta_info", {}
                            )
                            st.markdown(
                                f"**要約:** {meta_info_disp.get('summary', 'なし')}"
                            )
                            if meta_info_disp.get("mini_context"):
                                st.markdown(
                                    f"**GPT-mini用コンテキスト:** {meta_info_disp.get('mini_context')}"
                                )
                            st.text_area(
                                f"チャンク内容 (結果 {i_disp+1})",
                                res_detail_disp.get("text", ""),
                                height=200,
            ...
            if not client:
                ...
                with st.spinner("FAQを生成中..."):
                    count = generate_faqs_from_chunks(
                        selected_kb, int(max_tokens), int(num_pairs), client=client
                    )
    ...
    st.header(
        f"⟐ chatGPT - 現在の会話: {st.session_state.get('gpt_conversation_title', '新しい会話')}"
    )
    ...
    default_persona_name_display = (
        persona_display_names_chat_ui[0]
        if persona_display_names_chat_ui
        else "標準アシスタント（リスト空）"
    )
    current_persona_name_display_chat_val = next(
        (
            p["name"]
            for p in persona_details_list_chat_ui
            if p["id"] == current_persona_id_chat_val
        ),
        default_persona_name_display,
    )
    ...
    if persona_display_names_chat_ui:
        if current_persona_name_display_chat_val in persona_display_names_chat_ui:
            selected_persona_idx_chat = persona_display_names_chat_ui.index(
                current_persona_name_display_chat_val
            )

    selected_persona_name_chat_selected_ui = st.sidebar.selectbox(
        "AIペルソナ",
        persona_display_names_chat_ui,
        index=selected_persona_idx_chat,
        key="persona_chat_selectbox_main",
        help="AIの応答スタイルや性格を選択します。",
    )
    if persona_display_names_chat_ui:
        st.session_state["persona"] = persona_map_chat_ui[
            selected_persona_name_chat_selected_ui
        ]

    persona_data_chat_disp = load_persona(st.session_state["persona"])
    with st.sidebar.expander(
        f"ペルソナ「{persona_data_chat_disp.get('name')}」の詳細を見る", expanded=False
    ):
        st.write(f"**説明:** {persona_data_chat_disp.get('description')}")
        st.caption(
            f"**システムプロンプト:** {persona_data_chat_disp.get('system_prompt')[:100]}..."
        )

    temp_chat_val_ui = st.sidebar.slider(
        "温度 (創造性)",
        0.0,
        1.0,
    ...
    resp_len_idx_chat_val = (
        resp_len_options_chat_ui.index(current_resp_len_chat_val)
        if current_resp_len_chat_val in resp_len_options_chat_ui
        else 1
    )
    resp_len_chat_selected_ui = st.sidebar.radio(
        "応答の長さ",
        resp_len_options_chat_ui,
        index=resp_len_idx_chat_val,
        key="resp_len_chat_main",
        horizontal=True,
    )
    ...
    if user_chat_input_val:
        ...
        with st.chat_message("assistant"):
            ...
            if gpt_response_text_val:
                st.session_state.gpt_messages.append(
                    {"role": "assistant", "content": gpt_response_text_val}
                )
        ...
        if len(st.session_state.gpt_messages) >= 4 and client_for_chat_resp:
            ...
            if current_history_for_title_gen:
                try:
                    new_title_val = (
                        st.session_state.chat_controller.generate_conversation_title(
                            current_history_for_title_gen, client_for_chat_resp
                        )
                    )
    ...
    with st.sidebar.expander("過去の会話を管理する"):
        ...
        if saved_convs_list_ui:
            conv_options_display_load = [
                f"{conv.get('title', '会話')} ({conv['id'][-6:]} - {conv.get('date', '')[:10]})"
                for conv in saved_convs_list_ui
            ]
            conv_map_load_ids = {
                display_str: conv["id"]
                for display_str, conv in zip(
                    conv_options_display_load, saved_convs_list_ui
                )
            }
            selected_conv_display_str_load = st.selectbox(
                "読み込む会話を選択",
                conv_options_display_load,
                index=None,
...
st.markdown("---")
st.markdown(
    '<div class="footer-text">RAGシステム統合ツール v1.1.0 - AIによる知識活用を支援</div>',
    unsafe_allow_html=True,
)
