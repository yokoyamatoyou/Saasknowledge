...
class HybridSearchEngine:
    ...
    def reindex(self) -> None:
        ...
        if self.tokenized_corpus_file_path.exists():
            try:
                ...
                logger.info(f"Removed cached tokenized corpus: {self.tokenized_corpus_file_path}")
            except OSError as e:
    ...
    def _create_tokenized_corpus_and_filter_chunks(self) -> tuple[list[list[str]], list[dict]]:
        ...
        for i, chunk_data in enumerate(self.chunks):
            ...
            if tokens and not is_dummy_token_only:
                ...
                logger.warning(f"    警告: チャンクID {chunk_id} のトークン化結果がBM25に不適格。除外します。Tokens: {tokens}")
    ...
    def _load_or_build_bm25_index(self) -> typing.Union[BM25Okapi, None]:
        ...
        if not loaded_from_file:
            ...
            logger.info(f"    _load_or_build_bm25_index: self.chunks (before filter) = {len(self.chunks)}")
            self.tokenized_corpus_for_bm25, filtered_chunks = self._create_tokenized_corpus_and_filter_chunks()
            logger.info(f"    _load_or_build_bm25_index: filtered_chunks = {len(filtered_chunks)}")
